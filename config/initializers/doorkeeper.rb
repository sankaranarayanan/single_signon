Doorkeeper.configure do
  ...
  resource_owner_authenticator do |routes|
    ...
    User.find_by_id(session[:user_id]) || redirect_to(routes.signin_form_path(origin: request.fullpath))
  end
  ...
  #   # routes.new_admin_session_path
    session[:user_id] == 1 || redirect_to(routes.signin_form_path(return_to: request.fullpath))
  end
